...
void replaceLetters(const char* str) {
    ...
    *destPtr = '\0';
    cout << "Íîâàÿ ñòðîêà: " << destination << endl;
}
...
char* longestCommonSubstring(const char* str1, const char* str2) {
    ...
    int maxLength = 0;
    int endIndex = 0;
    // Ïðîõîäèì ïî êàæäîìó ñèìâîëó ïåðâîãî ñëîâà
    for (int i = 0; i < len1; i++) {
        for (int j = 0; j < len2; j++) {
            // Åñëè ñèìâîëû ñîâïàäàþò, ïðîâåðÿåì íà íàëè÷èå ïîäñòðîêè
            if (str1[i] == str2[j]) {
                int length = 0;
                const char* p1 = str1 + i;
                const char* p2 = str2 + j;

                // Ñðàâíèâàåì ïîäñòðîêè
                while (*p1 && *p2 && *p1 == *p2) {
                    length++;
                    p1++;
                    p2++;
                ...
                // Åñëè äëèíà ïîäñòðîêè áîëüøå ïðåäûäóùåé, îáíîâëÿåì
                if (length > maxLength) {
                    maxLength = length;
                    endIndex = i;
                }
    ...
    // Åñëè íå íàéäåíî îáùåé ïîäñòðîêè, âîçâðàùàåì nullptr
    if (maxLength == 0) {
        return nullptr;
    }
    char* commonSubstring = new char[maxLength + 1];
    strncpy(commonSubstring, str1 + endIndex, maxLength);
    commonSubstring[maxLength] = '\0'; // Çàâåðøàåì ñòðîêó
...
char* subString(char* words[10]) {
    ...
    int main() {
        ...
        cout << "Êîëè÷åñòâî ñëîâ, íà÷èíàþùèõñÿ ñ áóêâû a: " << numberWords(str, delimiters) << endl;
        cout << "Äëèíà ñàìîãî êîðîòêîãî ñëîâà: " << shortWord(str, delimiters) << endl;
        cout << "Êîëè÷åñòâî áóêâ a â ïîñëåäíåì ñëîâå: " << aCount(str, delimiters) << endl;
        replaceLetters(str);
        cout << "Êîëè÷åñòâî ñëîâ, ó êîòîðûõ ïåðâûé è ïîñëåäíèé ñèìâîëû ñîâïàäàþò: " << matchWord(str, delimiters) << endl;
        char* commonSubString = subString(words);
        if (commonSubString) {
            cout << "Ñàìàÿ êîðîòêàÿ îáùàÿ ïîäñòðîêà: " << subString(words) << endl;
        }
        else {
            cout << "Îáùàÿ ïîäñòðîêà íå íàéäåíà" << endl;
        }
        return 0;
    }